...
- [x] **普通碎片**：小怪依種類掉落，用於兌換點數、Cook Core 晶核
- [x] **Boss 碎片**：特定 Boss 專屬，用於轉職門檻與融合條件
- [ ] **保底機制**：連續若干波不掉落時保底給碎片（目前暫不設計）
- [x] Run 結算畫面新增「碎片獲得彙總」欄位
- [x] 碎片→點數 / Core 兌換介面（Shop 或 Lab 子頁面）
- [x] 材料上限與溢出轉換規則（各素材上限 9999，溢出自動轉換點數）

### 成長（天賦）系統（Talent Growth）
> 透過點數與碎片強化玩家角色基礎能力；全局持久。
- [x] 設計天賦樹結構：基礎生存支線、輸出支線、資源效率支線
- [x] 天賦節點資料 schema（`src/game/content/talents.ts`）
- [x] 天賦效果整合至 run 啟動時的角色 stats
- [x] 天賦 UI（新增 TalentScene 頁面）
- [x] 天賦重置機制（消費少量材料退還點數）
- [x] 確保天賦加成不破壞卡牌構築核心的前期壓力感
