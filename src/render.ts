...
export function drawWorld(
  ...
  for (const [, c] of world.with('enemy', 'weapon', 'pos')) {
    ...
    if (ang !== undefined && c.enemy!.bossPattern !== undefined) {
      ...
      for (let i = 0; i < n; i++) {
        ...
        g.lineTo(
          c.pos!.x + Math.cos(a) * RAY_LEN,
          c.pos!.y + Math.sin(a) * RAY_LEN,
        );
      }
  ...
  for (const [, c] of world.with('enemy', 'pos', 'radius')) {
    ...
    const bossStroke =
      kind === 'boss' && bossPattern === 'jets' && c.enemy!.bossEnraged
        ? 0xff3030
        : strokeColor;
...
function drawMatteFog(g: Graphics, theme?: StageTheme): void {
  ...
  g.fill({
    color: fogColor,
    alpha: Math.min(MAX_BASE_FOG_ALPHA, fogAlpha * FOG_ALPHA_SCALE),
  });
  const blobs = [
  ...
  for (const blob of blobs) {
    ...
    g.fill({
      color: fogColor,
      alpha: fogAlpha * FOG_BLOB_ALPHA_MULTIPLIER * blob.a,
    });
  }
}
