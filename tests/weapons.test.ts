...
import { applyCard, MAX_EXTRA_WEAPONS, POOL, type Card } from "../src/game/cards";
...
describe("Weapon cards: applyCard adds to extraWeapons", () => {
  ...
  it("respects MAX_EXTRA_WEAPONS soft cap", () => {
    ...
    expect(world.get(aid)!.avatar!.extraWeapons!).toHaveLength(MAX_EXTRA_WEAPONS);
  });
});
...
describe("Weapon mode behaviours", () => {
  ...
  it("burst fragments also spawn on ttl expiry", () => {
    ...
    const w: WeaponState = {
      mode: "burst", period: 1, damage: 4, projectileSpeed: 200,
      projectiles: 1, pierce: 0, crit: 0, cooldown: 0,
      ricochet: 0, chain: 0, burnDps: 0, burnDuration: 0, slowPct: 0, slowDuration: 0,
    };
...
describe("Mirror Boss reflects weapon picks", () => {
  it("each weapon card lifts boss damage / projectiles / hp / fire-rate off baseline", () => {
