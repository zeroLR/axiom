...
function placeContactEnemy(world: World, ax: number, ay: number, dmg = 1): number {
...
describe("Mirror Boss reflects evolution picks", () => {
  it("each evolution card lifts boss HP or speed off baseline", () => {
    const baseline = mirrorBossSpec([]);
    for (const cid of ["aegis", "revenant", "compact", "phaseShift"]) {
      const spec = mirrorBossSpec([cardById(cid)]);
      const stronger = spec.hp > baseline.hp || spec.maxSpeed > baseline.maxSpeed;
